-- Allow Splitwise-style split modes on expenses.
-- Run in Supabase SQL editor.

alter table public.split_expenses
  drop constraint if exists split_expenses_split_mode_check;

alter table public.split_expenses
  add constraint split_expenses_split_mode_check
  check (split_mode in ('equal', 'custom', 'exact', 'percentage', 'shares', 'adjustment'));

-- Keep create/update RPCs in sync (same mode list)
