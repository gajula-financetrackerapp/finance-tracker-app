...
/**
 * Browser Google OAuth with mail read — same path as Kashio sign-in.
 * Restores the existing Kashio session so connecting another inbox does not
 * switch the signed-in user.
 */
export async function connectGoogleGmailOAuth(): Promise<{
  email: string | null;
  accessToken: string | null;
  error: string | null;
}> {
  if (isExpoGo()) return { email: null, accessToken: null, error: 'GMAIL_NEED_BUILD' };
  if (!isSupabaseConfigured) return { email: null, accessToken: null, error: 'GMAIL_NEED_BUILD' };

  const { data: existing } = await supabase.auth.getSession();
  const restore = existing.session
    ? {
  ...
  const { data, error } = await supabase.auth.signInWithOAuth({
    ...
    options: {
      redirectTo,
      skipBrowserRedirect: true,
      scopes: `openid email profile ${GMAIL_READONLY}`,
      queryParams: {
  ...
  try {
    ...
    if (params.access_token && params.refresh_token) {
      retireCallbackUrl(opened.url);
      const set = await supabase.auth.setSession({
        access_token: params.access_token,
        refresh_token: params.refresh_token,
      });
      accessToken = set.data.session?.provider_token || null;
      email = (set.data.session?.user?.email || '').toLowerCase();
    } else if (params.code) {
      retireCallbackUrl(opened.url);
      const ex = await supabase.auth.exchangeCodeForSession(params.code);
      if (ex.error) {
        return { email: null, accessToken: null, error: 'GMAIL_DENIED' };
      }
      accessToken = ex.data.session?.provider_token || null;
      email = (ex.data.session?.user?.email || '').toLowerCase();
    } else {
      return { email: null, accessToken: null, error: 'GMAIL_DENIED' };
    }
    ...
    if (!accessToken) return { email: null, accessToken: null, error: 'GMAIL_SCOPE' };
    if (!email) return { email: null, accessToken: null, error: 'GMAIL_DENIED' };
    return { email, accessToken, error: null };
  } catch {
    return { email: null, accessToken: null, error: 'GMAIL_DENIED' };
  } finally {
